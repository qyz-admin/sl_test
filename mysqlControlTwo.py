...
class MysqlControl(Settings):
    ...
    def creatMyOrderT(self, team):
        ...
        yesterday = (datetime.datetime.now().replace(month=9, day=1)).strftime('%Y-%m-%d') + ' 00:00:00'
        # last_month = (datetime.datetime.now().replace(day=1) - datetime.timedelta(days=1)).strftime('%Y-%m') + '-01'
        last_month = (datetime.datetime.now().replace(month=9, day=1) - datetime.timedelta(days=1)).strftime('%Y-%m') + '-01'
        sql = '''SELECT a.id, 
        			a.month 年月, 
        			a.month_mid 旬, 
        			a.rq 日期, 
        ...
        print('正在获取' + (datetime.datetime.now().replace(month=8)).strftime('%m') + '月份订单…………')
        df = pd.read_sql_query(sql=sql, con=self.engine2)
        print('----已获取'+ (datetime.datetime.now().replace(month=8)).strftime('%m') + '月份订单…………')
    ...
    def creatMyOrderTT(self, team):
        ...
        last_month = (datetime.datetime.now().replace(day=1) - datetime.timedelta(days=1)).strftime('%Y-%m') + '-01'+ ' 00:00:00'
...
WHERE 
    ...
    def connectOrderT(self, team):
        ...
        today = (datetime.datetime.now().replace(month=8, day=31)).strftime('%Y-%m-%d')
        print('正在写入excel…………')
