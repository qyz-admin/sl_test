...
class MysqlControl(Settings):
    ...
    def connectOrder(self, team):
        ...
        ON a.`运单编号` = b.`运单编号`
				...
				'已退货(物流)', '已退货(不拆包物流)', '待发货转审核') AND a.日期>= '2020-10-01' AND a.日期 <= '2020-10-10'
				ORDER BY a.`下单时间` ;'''.format(team)
        print('正在获取10天的订单…………')
        df = pd.read_sql_query(sql=sql, con=self.engine1)
        today = datetime.date.today().strftime('%Y.%m.%d')
        print('正在写入excel…………')
        # df.to_excel('D:\\Users\\Administrator\\Desktop\\输出文件\\{} 神龙{}表.xlsx'.format(today, match[team]),
        #             sheet_name=match[team], index=False)
        # print('----已写入excel')
        print(df['订单编号', '币种', '最终状态'])
        print(df.groupby('最终状态').sum())
        # print(df.groupby('最终状态'))
        print(df['订单编号', '币种', '最终状态'].groupby('最终状态').sum())
        df.groupby('最终状态').to_excel('D:\\Users\\Administrator\\Desktop\\输出文件\\{} 神龙{}12表.xlsx'.format(today, match[team]),
                    sheet_name=match[team], index=False)
